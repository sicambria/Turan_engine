/* 2008 Jeff Morton (jeffrey.raymond.morton@gmail.com)
...
namespace SoundCatcher
{
    class AudioFrame
    {
        ...
        private bool IsDetectingEvents = false;
        public bool IsDetectingEvents_f
        {
            get { return IsDetectingEvents; }
            set { IsDetectingEvents = value; }
        }
        public bool IsEventActive = false;
        public bool IsAboveThreshold = false;

        private int AmplitudeThreshold = 16384;
        
        public int AmplitudeThreshold_f
        {
            get { return AmplitudeThreshold; }
            set { AmplitudeThreshold = value; }
        }

        private double currentAmplitude = 0;

        /// <summary>
        /// Gets or sets the current amplitude value.
        /// </summary>
        /// <value>The current amplitude value.</value>
        public double CurrentAmplitude_f
        {
            get { return currentAmplitude; }
            set { currentAmplitude = value; }
        }
        ...
        public void Process(ref byte[] wave)
        {
            ...
            if (_isTest == false)
            {
                ...
                for (int i = 0; i < wave.Length; i += 4)
                {
                    ...
                    CurrentAmplitude_f = Math.Abs(_waveLeft[h]);
                    if (IsDetectingEvents_f == true)

                        //IsEventActive = true;


                    if (Math.Abs(_waveLeft[h]) > AmplitudeThreshold)
                        IsEventActive = true;
