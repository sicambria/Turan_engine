...
namespace Felismero_motor
{
    public class H_FELDOLGOZO
    {
        ...
        public static double[,] create_window(double[] pcmdata, int num_of_frames)
        {
            ...
            for (int frame = 0; pos < pcmdata.Length; frame++)
            {
                ...
                else
                {
                    ...
                    {
                        window_array[frame-1, firsthalf] = window_array[frame - 1, from_prev_frame];
                    }
                    ...
                    for (int second_half = 128; second_half < 256; second_half++)
                    {
                        ...
                        {
                            window_array[frame-1, second_half] = pcmdata[pos++];
                        }
                    }
        ...
        public static void Show2dArray(double[,] two_dim_array, int arr_lines)
        {
            string temp = "";

            for (int i = 0; i < arr_lines; i++)  //arr_lines
            {
                for (int j = 0; j < mfcc_lpc_vect_num; j++)
                {
                    temp += ((int)two_dim_array[i, j]).ToString() + "\t";
                }
                temp += " --- /-/";
            ...
            dw.SetData(temp);
            dw.SetFormName(two_dim_array.GetLength(0).ToString() + " , " + two_dim_array.GetLength(1).ToString() + " , " + two_dim_array.Length.ToString());
            dw.Show();
        }

        public static void Show2dArray(int[,] two_dim_array, int arr_lines)
        {
            string temp = "";

            for (int i = 0; i < arr_lines; i++)  //arr_lines
            {
                for (int j = 0; j < two_dim_array.GetLength(1); j++)  // mfccnum
                {
                    temp += (two_dim_array[i, j]).ToString() + "\t";
                }
                temp += " --- /-/";
                temp += Environment.NewLine;
            }
        ...
        public static void ShowArray(double[] one_dim_array, int num_of_frames)
        {
            string temp = "";

            for (int i = 0; i < one_dim_array.Length; i++) //num_of_frames
            {
                for (int j = 0; j < mfcc_lpc_vect_num; j++) // formatting output
                {
                    temp += ((int)one_dim_array[i]).ToString() + "\t";
                }
                temp += " --- /-/";
                temp += Environment.NewLine;
            }
            ...
            DataView dw = new DataView();
            dw.SetData(temp);
            dw.SetFormName(one_dim_array.GetLength(0).ToString() + " , " + one_dim_array.Length.ToString());
            dw.Show();
        }
