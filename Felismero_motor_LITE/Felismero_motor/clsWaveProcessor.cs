...
public class clsWaveProcessor
{
    ...
    public bool StripStartEndOptimized(string strPath, bool trim1sec_fromstart)
    {
        if (strPath == null) strPath = "";
        if (strPath == "") return false;
        ...
        int window_silence_in = 30;
        int window_silence_out = 30;
        ...
        try
        {
            ...
            {
                short snd = ComplementToSigned(ref arrfile, j);
                short snd2 = ComplementToSigned(ref arrfile, j + window_silence_in);
                ...
                if (snd > FILTER_FREQ_LOW && snd < FILTER_FREQ_HIGH)
                {
                    ...
                    if (snd2 > FILTER_FREQ_LOW && snd2 < FILTER_FREQ_HIGH)
                    {
        ...
        for (int k = arrfile.Length - 1; k >= 0; k -= 2)
        {
            short snd = ComplementToSigned(ref arrfile, k - window_silence_out);
            short snd2 = ComplementToSigned(ref arrfile, k - 1);
